...
router.post('/register', async (req, res) => {
    try {
        ...
        const result = await prepare('INSERT INTO users (username, email, password, role, full_name) VALUES ($1, $2, $3, $4, $5) RETURNING id').run(username, email, hash, role, full_name || username);
...
router.post('/login', loginLimiter, async (req, res) => {
    try {
        ...
        const user = await prepare('SELECT * FROM users WHERE username = $1 OR email = $2').get(username, username);
...
router.get('/me', requireAuth, async (req, res) => {
    try {
        const user = await prepare('SELECT id, username, email, role, full_name, created_at FROM users WHERE id = $1').get(req.user.id);
...
// GET /api/auth/verify — validate token, return user info (used by frontend auth checks)
router.get('/verify', requireAuth, async (req, res) => {
    try {
        const user = await prepare('SELECT id, username, email, role, full_name FROM users WHERE id = $1').get(req.user.id);
        if (!user) return res.status(404).json({ error: 'User not found' });
